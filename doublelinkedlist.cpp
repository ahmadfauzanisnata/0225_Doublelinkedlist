...
void addNode()
{
    ...
    newNode->next = START; // step 3: make the new node point to the new first node
    if (START != NULL)
    {
    ...
    else
    {
        ...
        Node *current = START; // step 1.a: start from the fisrt node
        Node *previous = NULL; // step 1.b: previous node is NULL initially

        while (current != NULL && current->noMhs)
        {                               // step 1.c: traverse the list to find the correct position
            previous = current;        // step 1.d: move the previous to the current position
      ...
      if (current != NULL)
        ...
        if (previous != NULL)
        {
            previous->next = newNode; // step 7: make the next field of the previous node point tothe new node.
        }
        else
        {
            // if previous is still NULL, it means newNode is now the fisrt node
            START - newNode;
        }
...
bool search(int rollNno, Node **previous, Node **current)
{
    *previous = NULL;
    *current = START;
    while (*current != NULL && (*current)->noMhs != rollNno)
    {
        *previous = *current;
...
void deleteNode()
{
    ...
    cout << "\Enter the roll number of the student ehose record isto be deleted: ";
    ...
    if (current == NULL)
    {
        cout << "\033{32mThe record with roll number " << rollNo << " not  found\033]0m" << endl;
        return;
    }
    ...
    if (current == START)
    {
        ...
        {// if there's a succesor, updste its prev pointer
            current->next->prev = previous;
        }
    }
    // relase thhe memory of the node marked as current
    delete current;
...
void traverse()
{
    ...
    cout << "\nList is empty" << endl;
        ...
        while (currentNode != NULL) //step 2
        {
            ...
            currentNode = currentNode->next;                                // step4
        }
    }
}
void revtraverse()
